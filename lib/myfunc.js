...
exports.GIFBufferToVideoBuffer = async (image) => {
        ...
        const filename = `${Math.random().toString(36)}`
                await fs.writeFileSync(`./trash/${filename}.gif`, image)
                         child_process.exec(
                                    `ffmpeg -i ./trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./trash/${filename}.mp4`
                                            ) 
      await sleep(4000)
      
        var buffer5  =  await  fs.readFileSync(`./trash/${filename}.mp4`)
        Promise.all([unlink(`./trash/${filename}.mp4`), unlink(`./trash/${filename}.gif`)])
        return buffer5
                       }
/**
 * Serialize Message
